...
主 Agent 仍按微服务业务 payload 区分 `success`、`degraded`、`unsupported`、`failed`。在生成前，端工具权限明确拒绝时使用独立的“权限不可用”终止话术；其它非完整满足或异常回复收敛为“部分数据不支持、整体不支持、其它异常”三类固定话术。不要复述内部候选计划、schema、CardSpec、DSL、来源 URL 或校验细节。
...
| 权限不可用 | `RequestDataPermission` 返回 `result.stateOfPermission: "false"` | 否，立即终止并使用权限固定话术 |
...
- 当前工具快照中只将 `result.stateOfPermission` 的精确字符串 `"true"` 视为通过，精确字符串 `"false"` 视为权限不可用；缺失、类型错误、其它值、调用失败或工具不可用均归为其它异常，不调用生成工具。
...
条件：`RequestDataPermission` 返回 `result.stateOfPermission: "false"`。
...
- 权限不可用话术和三类固定话术不得改写同义句、增加前后缀或拼接工具自定义文案。
