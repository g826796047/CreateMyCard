...
本文档维护模式分支、候选构造和工具契约；用户输出统一遵守 [Skill 用户输出优先规则](../SKILL.md#用户输出优先规则)，正常话术见其回复执行链，必要例外见异常与决策回复。一次任务只读取一次本文档；示例和静态工具快照仅用于用户明确要求的联调、排障或回归，且不能覆盖当前运行时 schema。本文中的“停止、终止、结束”均指停止后续业务工具，先按主文件对应节点或例外发送说明，再结束本轮；需要追问时发送问题后等待。

执行节点、回复检查和“已回复”判据统一见 [Skill 回复执行链](../SKILL.md#回复执行链)。本指南的判断、校验和参数准备均在内部静默完成，不向用户复述。
...
纯视觉 edit 可跳过 overview/schema；删除数据或修改已有数据参数的 edit 获取本轮 overview，并为保留数据加载 schema。
零工具的分流例外按主文件发送对应说明，不发送开始消息。
...
外部来源采用主文件节点 3；生成返回后执行文末“生成结果处理”，再按主文件节点 5 回复并结束。
...
- 每个来源串行调用，校验结构、类型、语义与相关性。输出时机与事实说明遵守
  [获取外部内容与来源说明](../SKILL.md#3-获取外部内容与来源说明)；失败按核心或次要来源处理。
  前一个合法事实仅在来源说明发出后可用于后续来源入参。外部 Skill 内部辅助调用不逐步播报。
...
- 核心来源失败先按异常与决策回复说明再停止；次要来源失败先说明移除项，再删除相关 query 片段、候选参数或动作动态值。
...
- `stateOfPermission:false` 或任一 `authorized:false` 一票否决并终止生成，必须按 Skill 异常与决策回复的权限未通过行回复，不得调用生成工具、追问、建议或改写话术。
- `nonAuthStatus` 非空时，每项必须是对象且 `name` 为非空字符串；`settingsPath` 缺失按空字符串。任一有效项即终止生成，必须按 Skill 异常与决策回复的权限未通过行逐项回复；同名项保留第一项，不输出 capabilityId、authType 或 authorized。
...
- 工具正常返回但缺少 `result`、`stateOfPermission` 非 Boolean 或明细非法时按结果非法终止，使用 Skill 异常与决策回复的异常行；这不是调用失败，不适用默认开启。
...
5. 将判断结果交给 [结果判断、最终说明与结束](../SKILL.md#5-结果判断最终说明与结束)，执行最终回复检查。
