...
R16/R16W 仅用于尚未告知的采用事实；没有新增待告知事实时省略该消息，工具顺序和参数来源不变。
...
| --- | --- | --- | --- |
| W01 | Skill 前 W0 已提供充分且尚未告知的相关事实，无动态数据 | W0 → 加载 Skill → R01 → O → R16W → G → R18C | 不重新搜索，不发送搜索进度；事实直接呈现，结果只说明生成与用途 |
| W02 | W0 充分，且此前助手已告知全部采用事实 | W0 → 先前说明 → 加载 Skill → R01 → O → S/P（按数据）→ G → R18C | 不重复 R16W，不复述搜索数据值；真实来源仍取 W0 |
| W03 | W0 有多条结果，其中部分事实已告知 | R01 → 必要前置 → 校验采用 → 一次合并 R16W → G → R18C | R16W 仅含尚未告知的采用事实；必要时间、地点和信源名称直接附在事实旁 |
| W04 | W0 只覆盖部分事实，还需补查 | R01 → 必要前置 → R16W（尚未告知的已有事实）→ W → R16W（尚未告知的新事实）→ G → R18C | 补查在权限后且仅覆盖不足部分；不发送进度，新事实在生成前实际说明 |
...
| W17 | 前置来源已将采用事实完整告知，但卡片尚未生成 | R01/R02 → 必要卡片步骤 → G → R18C/R18E（尚需结果反馈时） | 不重复事实，不把搜索完成等同于卡片完成；仍检查真实来源和必要参数 |
...
| 场景 | 预期顺序 | 约束 |
| --- | --- | --- |
| Skill 前已有外部事实 | R01/R02 → 动态能力工具（如需）→ 权限（仅有动态集合时）→ 校验采用 → R16/R16W → G → R18C/R18E | 不因取得时点跳过告知；静态事实写入 userQuery 或合法业务参数 |
| Skill 执行中取得非 WebSearch 外部事实 | 必要前置 → 来源返回 → 校验采用 → R16 → G → 结果回复 | 不注册动态能力、不伪造刷新能力 |
| 纯静态外部卡片 | R01 → O → 无 S/P → 校验采用 → R16/R16W → G → R18C | 外部静态内容可独立满足核心用途 |
| 动态与外部静态混合 | R01 → O → S → P → 校验采用 → R16/R16W → G → R18C/R18E | 权限只约束动态集合，外部事实仍须生成前告知 |
