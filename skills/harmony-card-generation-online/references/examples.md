...
示例中的“结果播报”和“最终回复”都是独立的用户消息，不是工具返回字段或调用日志。按主文件回复执行链执行：
正常情况只发送开始、有效来源内容和最终说明，其余步骤静默；检查本身不输出，不等待未定义的发送确认。

以下轨迹同时包含回复和调用。`开始`表示一次开始回复；`最终回复`表示生成结果之后的用户可见文本，实际发送后才结束。
中途不支持、失败或待确认也要先说明或追问，再结束或等待，使用 [Skill 异常与决策回复](../SKILL.md#异常与决策回复)。
正常话术见 [Skill 回复执行链](../SKILL.md#回复执行链)。以下执行轨迹仅用于联调，不向用户复述。
...
本例假设来源实际返回并校验了“今晚 19:30、上海星光剧场”，以下播报仅使用该受控事实。
...
R01—R13 每组在修改前后各至少回放 10 次，每版至少 130 次；一组包含多个分支时，每个分支各至少 10 次。
...
| R13 | 分别回放无来源、单来源、双来源，以及一个外部 Skill 内含多次辅助调用的成功场景；采用与主文件不同的来源显示名和受控事实 | 分别只有 2、3、4、3 条消息；静默完成内部步骤与辅助调用，无技能完成额外总结；来源说明含真实受控事实 |

除场景专属断言外，统一检查；其中 N＋2 数量约束仅适用于无必要例外的正常成功 create/edit：

- N 个有效外部来源恰好输出 N＋2 条消息；内部卡片工具和外部 Skill 内部辅助调用不计为独立来源。
- 工具前后及工具间不播报计划、判断依据、校验报告或回复检查；来源获取前不播报，生成前不重复提示。
- 同一条允许的消息也不得夹带额外过程说明，不能仅靠消息总数判定通过。
- 来源说明使用当前回放结果的具体事实；只说“已搜索”或“已获取时间和地点”判为内容不合规。
- 明确触发的追问、替代确认、移除、权限拒绝、不支持和失败单独按例外判定；正常检查通过不算例外。
...
| 指标 | 计算口径 |
| --- | --- |
| 开始说明覆盖率 | 首个业务工具前实际展示对应开始说明的会话数 / 本轮进入业务工具链的会话数 |
...
| 额外播报率 | 存在未获允许的过程消息，或允许消息夹带过程文本的正常成功会话数 / 正常成功会话数 |
| 消息内容合规率 | 时机对应且内容符合该节点要求的已展示消息数 / 已展示助手用户消息数（包含多余消息） |
| 顺序违规率 | 发生抢先调用、抢先结束或合并来源播报与最终说明的会话数 / 适用会话数 |
| 重复回复率 | 出现重复开始回复、同一来源重复播报或同一生成结果重复最终说明的会话数 / 适用会话数 |

另外保留越界承诺率，并逐条记录重复生成、失败误报成功和内部信息泄露样本。
默认验收目标：开始说明、来源播报、生成后最终回复覆盖率分别达到 95% 以上，较基线不下降；
正常成功链路额外播报率、顺序违规率、重复回复率目标均为 0，已输出消息内容须全部合规。
不得引入重复生成、失败误报成功或内部信息泄露。成功摘要内容、顺序及重复回复仍须逐条检查，
不能因非空文本达标就宣称全部遵从性通过。

多余播报需记录其发生节点、原文及来源：区分主模型额外生成文本、来源 Skill 的用户消息、运行时自动展示文本。
若宿主更高层指令要求逐步汇报，记录实际冲突，不以 Skill 的优先声明声称覆盖它。
