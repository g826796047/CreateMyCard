...
- **来源与检查：** 数据 ID/参数名/类型/路径来自本轮定义，事件完整复制 actionTemplate，仅替换声明的动态参数，
  素材只传 ID；静态业务值可来自用户输入或已校验外部事实。外部事实仅写入有效 userQuery 或现有业务参数；有效标题和说明只表达保留需求。
...
- **已有结果：** 先检查步骤 1 识别的当前任务真实外部来源结果，即使搜索发生在 Skill 加载前也必须处理。
...
```text
已有外部来源：从当前任务真实工具轨迹读取结果 → 步骤 7 校验、回填 → R16W（有尚未告知的采用事实时）
