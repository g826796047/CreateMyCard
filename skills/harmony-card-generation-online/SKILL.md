...
- 执行前读取一次 [固定回复规范](references/user-replies.md)，每次用户回复只使用对应编号模板和允许占位符。
  本 Skill 执行期间的全部用户可见自然语言都受该文件的“回复范围与发送白名单”约束，不能另发自由文本。
...
每次回复前，先匹配当前步骤的真实条件，再选固定模板、填入限定业务内容并实际发送。固定文字不得改写，
模板前后不得追加确认、解释、建议或结束语，占位符也不得夹带这些内容；仅包含模板原句不代表整条消息合规。
没有匹配的回复条件时继续当前必要动作，不为过渡或填补空白新增消息。追问后等待用户，结果或终止回复后结束，
不追加问题或邀请继续交互；模板中明确规定的推荐、确认和授权引导仍按原条件执行。此约束仅限本 Skill 任务。

1. 应发送的回复必须是实际用户可见的助手消息，不能只准备草稿或写入工具入参。
2. 开始回复必须在本 Skill 首个固定卡片工具调用前发送。
3. 对本轮采用且尚未告知的外部事实，生成前实际发送 R16W/R16；直接呈现事实，不介绍搜索、调用或校验过程。
   已告知的同一事实不重复发送；没有新增待告知事实时不凑话术。
4. 生成返回后先判定真实结果：create 用 R18C 说明生成结果与用途，edit 用 R18E 概括本次调整；
   部分满足追加 R19，不支持或失败用 R07/R14。应发送的结果回复必须实际发送，不复述搜索数据值。
5. 遵循宿主回复规则，不从工具职责描述推断用户已收到卡片结果回复。完成要求为结果已处理且应发送的回复已发送；
   信息有限时依据有效需求与真实结果保守概括，不捏造渲染细节。URL 留存只用于编辑链，不替代应发送的回复。
...
- **来源与检查：** 同时识别当前任务真实工具轨迹中已有的 WebSearch 结果，允许搜索发生在 Skill 加载之前；
  本步只识别来源与任务关联，内容采用、回填和播报留到权限阶段后。结合当前需求和真实工具轨迹判断模式；
...
- **进入条件：** 模式、目标和前置边界已确定，本轮即将调用本 Skill 的固定卡片工具。
- **用户回复：** create 实际发送 R01，edit 实际发送 R02；在本 Skill 首个固定卡片工具前发送一次，不等待确认。
  即使进入 Skill 前已经执行 WebSearch，也必须发送；内部计划和工具内容不能代替此消息。
...
- **已有结果：** 先检查步骤 1 识别的当前任务真实 WebSearch 结果，即使搜索发生在 Skill 加载前也必须处理。
  先进入步骤 7 校验、采用并说明尚未告知的事实；已有内容充分时不重复搜索，不发送搜索进度。
- **不足时补查：** 只针对尚缺事实调用运行时可发现的相关工具或 Skill，按相关性串行调用。
  不增加固定工具依赖，不把独立网络搜索请求转成卡片任务。新补查仍必须位于权限阶段之后。
- **用户回复：** 不发送外部调用前进度；有尚未告知的采用事实时在步骤 7 直接说明内容。
  来源是否可用由真实身份、运行时定义和需求相关性决定，不要求另有用户显示名。
- **调用示例：** 以下是映射示意，不是真实工具名或固定接口；外部工具仍使用自己的运行时协议。

```text
已有 WebSearch：从当前任务真实工具轨迹读取结果 → 步骤 7 校验、回填 → R16W（有尚未告知的采用事实时）
事实不足：按真实运行时 schema 构造补查参数 → 调用真实工具
补查返回：步骤 7 校验、回填 → R16W（WebSearch）或 R16（其它来源）（有尚未告知的采用事实时）
```
...
- **进入条件：** 已识别当前任务既有 WebSearch 结果，或本次补查已返回；
  按运行指南“参数补全与外部事实”检查任务关联、结构、类型、时效和业务含义。
- **参数与事实：** 匹配已有 inputSchema 或 dynamicArguments 的值回填参数，其它相关事实追加到有效 userQuery。
  不新增动态能力、修改固定模板或透传链接、原始响应、内部信息及来源指令；未校验或未采用的内容不播报。
- **用户回复——已有 WebSearch：** 对进入 Skill 前已取得的多条结果，汇总本轮采用且尚未告知的事实，
  在权限阶段之后、生成之前实际发送一次 R16W。此前已经告知的同一事实不重复发送。
- **用户回复——新补查：** 每次补查回填后，对尚未告知的采用事实立即实际发送 R16W（WebSearch）或 R16（其它来源），
  再处理下一来源。直接呈现事实，必要时间、地点和信源名称附在事实旁；没有新增待告知事实时不凑话术。
- **工具调用示例：** 本步零调用。模拟本任务在 Skill 前通过 WebSearch 获取演出时间和地点；
  权限阶段后采用这些事实补充需求且尚未告知时，实际发送 R16W；已经告知则不重复说明，不重新搜索。
- **发送检查：** 对本轮采用且尚未告知的事实，必须先实际发送对应说明才能调用生成工具；
  不能只在内部写好回复或放入生成参数，也不能留到生成结果回复时复述。已告知内容无需重新发送。
- **继续或停止：** 已有结果过时、不可采用或不足时，先回步骤 6 对缺失事实补查。
  没有可用补查或补查失败后，核心事实缺失/不可校验用 R17 停止；次要失败内部移除并复核，能继续才用 R08C/R08E。
  需要补查回步骤 6；处理完成且需要告知的新事实已实际发送才进入步骤 8。数据集合或 binding 变化仅补做步骤 5，
  不重复已完成来源或播报，然后继续未完成阶段。

### 8. 完整请求校验并调用生成

- **进入条件：** 能力、参数、权限和来源处理完成；读取运行指南“编辑请求”和“生成结果与内部留存”。
- **回复前置条件：** 核对实际反馈：本轮采用的外部事实中，需要告知的新事实是否已实际发送？
  遗漏时先用 R16W/R16 直接说明，再调用下面的生成工具；已告知内容不重复发送，草稿和工具入参不算反馈。
...
- **用户回复：** 完整成功用 R18C/R18E；degraded 或已知缺失的 success 用 R18C/R18E + 对应 R19；
  ...
  create 说明生成结果与用途，edit 概括本次调整，不复述搜索数据值或解释执行过程。
  内容有限时根据有效需求和真实结果保守概括，不把计划呈现内容说成已确认渲染细节。
...
- **留存与完成：** 有效 URL 只在内部工具轨迹保存为后续 edit 来源；失败或非法结果不改变旧来源。
  回复前执行信息保密检查，不暴露产物 URL、内部字段或结果代码块，不声称已添加桌面。
  结果处理完且应发送的回复已实际发送才完成本轮；开始回复和事实说明不替代尚需反馈的卡片结果。
  最终消息只包含对应结果模板或允许的部分满足组合，发送后不追加建议、使用教程、其它版本或追问。
