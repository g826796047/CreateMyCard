...
本文档是 AI 卡片云侧方案的唯一权威来源。`AGENTS.md` 只保留项目背景、关键文件和协作规范，不再维护接口、状态码、CardSpec、TaskSpec、Artifact、校验、降级等方案细节。
...
`dataModel.value` 根据最终 CardSpec 和能力 `outputSchema` 生成，必须包含必要根结构和加载态：
